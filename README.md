...
The repository contains firmware, an Android/Kotlin app, Python data utilities, and software-only test paths. BLE dataset protocol v3 is the current source-compatible pair: the firmware and Android app must be upgraded together.

Current project status:

- BLE v3 dataset recording, catalog, resumable download, CRC32 verification, and guarded deletion are implemented in source.
...
- A v3 mock device simulates the dataset workflow when the board is unavailable.
- The recovered v3 implementation has not yet been verified end-to-end on the repaired physical prototype. Software builds and tests do not prove BLE radio, flash, power-loss, or battery behavior.
...
- provides BLE v3 status, recording, catalog, resumable download, cancel, and guarded delete operations
...
## BLE Dataset Protocol v3

The authoritative wire contract is [docs/ble_protocol_v3.md](docs/ble_protocol_v3.md). Protocol v3 is not wire-compatible with the earlier v1/v2 prototypes. Firmware and Android must use the same version.
...
2. Connect to the board and wait for the v3 `hello` capability check and `status` reconciliation.
...
5. Tap `Stop`. Firmware closes and finalizes the active CSV, then returns its name, byte size, and CRC32.
6. Android automatically starts or resumes the download. The `.part` file is promoted to the final `.csv` only when both size and CRC32 match.
7. Deletion is never automatic. The user may manually delete the board copy only after local verification and an explicit confirmation dialog. The delete command repeats the expected name, size, and CRC32.
...
The v3 code can be exercised without a board as described in [docs/testing_without_hardware.md](docs/testing_without_hardware.md). Physical BLE, QSPI persistence, disconnect range, power-loss recovery, and real transfer throughput remain pending hardware tests.
...
- BLE scan/connect, v3 handshake, status reconciliation, indications, notifications, and queued command writes
- a `Data` screen for selecting the recorded activity, start/stop, storage status, log catalog, verified download/resume, and confirmed deletion
- automatic post-stop CSV download to a user-selected Storage Access Framework folder
- byte-count and CRC32 verification before `.part` is finalized
- a full v3 mock device for recording/catalog/download/delete development without the board
...
- v3 has not yet been validated end-to-end on the repaired physical board
...
### v3 simulator without the board
...
6. Exercise disconnect/reconnect, cancel/resume, and the manual delete confirmation.
...
The v3 integration also requires a host-native protocol test environment:
...
The normal firmware accepts the same newline-delimited v3 control records over
...
BLE v3 dataset labels are limited to:
...
No research dataset has been collected yet. Before recording data intended for ML, first complete the physical v3 validation, choose a stable mount and orientation, and define the measurement protocol. Future recordings should use:
...
compatible with the current firmware. BLE v3 intentionally carries file bytes
only through the binary `file_data` characteristic; do not use the legacy tool
for new recordings or as evidence that a v3 transfer was verified.
...
1. complete native/JVM/emulator verification of protocol v3
2. validate BLE v3 and QSPI behavior on the repaired physical prototype, including reconnect, power loss, resume, CRC mismatch, and guarded deletion
